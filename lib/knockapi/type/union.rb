...
module Knockapi
  module Type
    ...
    module Union
      ...
      # @api private
      #
      # @param key [Symbol, Hash{Symbol=>Object}, Proc, Knockapi::Type::Converter, Class]
      #
      ...
      private def variant(key, spec = nil)
        variant_info =
          case key
          in Symbol
            [key, Knockapi::Type::Converter.type_info(spec)]
          in Proc | Knockapi::Type::Converter | Class | Hash
            [nil, Knockapi::Type::Converter.type_info(key)]
          end
      ...
      # @param value [Object]
      #
      # @return [Knockapi::Type::Converter, Class, nil]
      ...
      def coerce(value, state:)
        if (target = resolve_variant(value))
          return Knockapi::Type::Converter.coerce(target, value, state: state)
        end
        ...
        known_variants.each do |_, variant_fn|
          ...
          coerced = Knockapi::Type::Converter.coerce(target, value, state: state)
      ...
      def dump(value)
        if (target = resolve_variant(value))
          return Knockapi::Type::Converter.dump(target, value)
        end

        known_variants.each do
          target = _2.call
          return Knockapi::Type::Converter.dump(target, value) if target === value
        end

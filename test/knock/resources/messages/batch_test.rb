# frozen_string_literal: true

require_relative "../../test_helper"

class Knock::Test::Resources::Messages::BatchTest < Knock::Test::ResourceTest
  def test_archive_required_params
    skip(
      "skipped: currently no good way to test endpoints defining callbacks, Prism mock server will fail trying to reach the provided callback url"
    )

    response = @knock.messages.batch.archive(message_ids: ["182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"])

    assert_pattern do
      response => ^(Knock::ArrayOf[Knock::Models::Messages::BatchArchiveResponseItem])
    end
  end

  def test_get_content_required_params
    skip(
      "skipped: currently no good way to test endpoints defining callbacks, Prism mock server will fail trying to reach the provided callback url"
    )

    response = @knock.messages.batch.get_content(message_ids: ["string"])

    assert_pattern do
      response => ^(Knock::ArrayOf[Knock::Models::Messages::BatchGetContentResponseItem])
    end
  end

  def test_mark_as_interacted_required_params
    skip(
      "skipped: currently no good way to test endpoints defining callbacks, Prism mock server will fail trying to reach the provided callback url"
    )

    response = @knock.messages.batch.mark_as_interacted(message_ids: ["1jNaXzB2RZX3LY8wVQnfCKyPnv7"])

    assert_pattern do
      response => ^(Knock::ArrayOf[Knock::Models::Messages::BatchMarkAsInteractedResponseItem])
    end
  end

  def test_mark_as_read_required_params
    skip(
      "skipped: currently no good way to test endpoints defining callbacks, Prism mock server will fail trying to reach the provided callback url"
    )

    response = @knock.messages.batch.mark_as_read(message_ids: ["182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"])

    assert_pattern do
      response => ^(Knock::ArrayOf[Knock::Models::Messages::BatchMarkAsReadResponseItem])
    end
  end

  def test_mark_as_seen_required_params
    skip(
      "skipped: currently no good way to test endpoints defining callbacks, Prism mock server will fail trying to reach the provided callback url"
    )

    response = @knock.messages.batch.mark_as_seen(message_ids: ["182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"])

    assert_pattern do
      response => ^(Knock::ArrayOf[Knock::Models::Messages::BatchMarkAsSeenResponseItem])
    end
  end

  def test_mark_as_unread_required_params
    skip(
      "skipped: currently no good way to test endpoints defining callbacks, Prism mock server will fail trying to reach the provided callback url"
    )

    response = @knock.messages.batch.mark_as_unread(message_ids: ["182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"])

    assert_pattern do
      response => ^(Knock::ArrayOf[Knock::Models::Messages::BatchMarkAsUnreadResponseItem])
    end
  end

  def test_mark_as_unseen_required_params
    skip(
      "skipped: currently no good way to test endpoints defining callbacks, Prism mock server will fail trying to reach the provided callback url"
    )

    response = @knock.messages.batch.mark_as_unseen(message_ids: ["182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"])

    assert_pattern do
      response => ^(Knock::ArrayOf[Knock::Models::Messages::BatchMarkAsUnseenResponseItem])
    end
  end

  def test_unarchive_required_params
    skip(
      "skipped: currently no good way to test endpoints defining callbacks, Prism mock server will fail trying to reach the provided callback url"
    )

    response = @knock.messages.batch.unarchive(message_ids: ["182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e"])

    assert_pattern do
      response => ^(Knock::ArrayOf[Knock::Models::Messages::BatchUnarchiveResponseItem])
    end
  end
end
